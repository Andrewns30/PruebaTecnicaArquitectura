...
namespace PruebaTecnica.Helpers.Extensions
{
    public static class ResponseServiceExtensions
    {
        ...
        public static async Task<ResponseServiceDto<T>> GetResultWithResult<T, TResult>(this ResponseServiceDto<T> responseServiceDto, ResponseServiceDto<TResult> result)
        {
            return await Task.Run( () =>
            {
                responseServiceDto.Code = result.Code;
                responseServiceDto.Message = result.Message;
                return responseServiceDto;
            });           
        }
